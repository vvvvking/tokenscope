/**
 * TokenScope proxy — store.mjs
 *
 * NDJSON ring buffer on disk. One line per call record, keyed chronologically.
 * Kept tiny: no index, no transactions — we linearly scan for queries, which
 * is fine for the ≤ 5k record retention we expect per user.
 *
 * File: ~/.tokenscope/records.ndjson
 */
...
export class Store {
  ...
  async append(record) {
    ...
    if (this._writesSinceCompact >= 200) {
      this._writesSinceCompact = 0;
      this.compact().catch(e => console.warn('[TS-proxy] compact failed:', e.message));
    }
  }
